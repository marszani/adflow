...
&           gamma_effd
        ...
        call popcontrol1b(branch)
        if (branch .eq. 0) then
          tempd3 = max1d/(3.235*re_theta_c)
          re_sd = tempd3
          re_theta_cd = -(re_s*tempd3/re_theta_c)
        else
          re_theta_cd = 0.0_8
          re_sd = 0.0_8
        end if
...
&           *2*(rlmce2*w(i, j, k, itransition1)-1.0)*x1d/(rlmce2-1)**2
        ...
        temp2 = -(temp3**4)
        f_waked = exp(temp2)*x4d
        tempd3 = -(4*temp3**3*exp(temp2)*f_wake*x4d/delta)
        deltad = -(temp3*tempd3)
        temp3 = w(i, j, k, irho)
        temp2 = temp3*(u*u)
        temp1 = vort*rlv(i, j, k)/temp2
        temp = w(i, j, k, itransition2)
        tempd2 = temp1*375.0*deltad
        tempd0 = temp*d2wall(i, j, k)*375.0*deltad/temp2
        vortd = rlv(i, j, k)*tempd0
        tempd1 = -(temp1*tempd0)
        wd(i, j, k, irho) = wd(i, j, k, irho) + u**2*tempd1
        ud = 2*u*temp3*tempd1
...
&         , j, k)*tempd2
        ...
        temp2 = sqrt(temp3)
        temp1 = d2wall(i, j, k)
        d2walld(i, j, k) = d2walld(i, j, k) + tempd3 + temp*tempd2 + 2*&
&         temp1*temp0*temp2*re_sd
        tempd2 = temp1**2*temp2*re_sd/rlv(i, j, k)
        rlvd(i, j, k) = rlvd(i, j, k) + vort*tempd0 - temp0*tempd2
